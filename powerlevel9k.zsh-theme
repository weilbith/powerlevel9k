...
prompt_docker_machine() {
  ...
  if [[ -n "$docker_machine" ]]; then
    "$1_prompt_segment" "$0" "$2" "magenta" "$DEFAULT_COLOR" "$docker_machine" 'DOCKER_ICON'
  fi
}
...
$(print_icon 'MULTILINE_LAST_PROMPT_PREFIX')'
    ...
    PROMPT='%f%b%k%B$(build_left_prompt)%b'
    RPROMPT_PREFIX=''
    RPROMPT_SUFFIX=''
  fi

  if [[ "$POWERLEVEL9K_DISABLE_RPROMPT" != true ]]; then
    RPROMPT='$RPROMPT_PREFIX%f%b%k%B$(build_right_prompt)%b%{$reset_color%}$RPROMPT_SUFFIX'
  fi
NEWLINE='
'
